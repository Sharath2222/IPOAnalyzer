...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchData, 5 * 60 * 1000); // Set up periodic refresh (every 5 minutes)
  ...
  const fetchData = async () => {
    try {
      ...
      await fetchIpoData();
      // You may add a similar function in CurrencyExchange component
      // and call it here to refresh exchange rates
    } catch (error) {
